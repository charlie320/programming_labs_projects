class UsersController < ApplicationController
  ...
  def show
    @user = current_user
    # @my_playlist = @user.songs.group(:id)
    @my_playlist = Song.joins(:user_playlists).where('user_playlists.user_id = ?', [@user.id]).select('songs.*, count(user_playlists.id) as number').group(:id)
  end
